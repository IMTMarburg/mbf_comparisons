...
class DESeq2MultiFactor:
    ...
    def prefixes(
        ...
        main_levels = [level for level in df_factors[main_factor] if level != main_reference]
        ...
        if len(interactions) == 0:
            ...
            for main_level in main_levels:
                ...
                for factor in other_factors:
                    ...
                    prefixes.append(prefix)
                    levels = [level for level in df_factors[factor] if level != reference]
    ...
    def compare(
        ...
        for factor in factor_reference:
            ...
            replace = [f"z{level}" if level != ref_level else f"b{level}" for level in df_factors[factor]]
...
class NOISeq:
    ...
    lc : int, optional
        Wether length normalization is performed (rpkm), by default 0.
    v : float, optional
